...
function query_delete_friend(username, friend_username, call_back) {
    data.query_with_values('DELETE FROM friends WHERE username = \'' + username + '\' AND friend_username = \'' + friend_username + '\'', [], function() {
        call_back()
    })
}
...
function remove_friend(data, socket) {
    let username = data.username
    let friend_username = data.friend_username
    query_delete_friend(username, friend_username, function() {
